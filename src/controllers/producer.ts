...
export const producer = async (req: Request, res: Response) => {
  try {
    const { producer } = req.params;
    const { page } = req.query;
    const response = await axios.get(`${process.env.BASE_URL}/producer/${producer}?page=${page}`);
    const html = response.data;
    const $ = cheerio.load(html);

    const data: ProducerTypes[] = []
